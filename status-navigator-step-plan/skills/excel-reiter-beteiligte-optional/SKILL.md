---
name: excel-reiter-beteiligte-optional
description: "Fuegt optional einen Reiter Beteiligte hinzu: Uebersicht aller Parteien mit Rolle, Funktion, Anteilen und Kontaktdaten. Vorbereitung fuer die spaetere Zustellung und Kommunikation."
---

> **Hinweis:** Plugin `status-navigator-step-plan`, Dokumentenverarbeitung
> ohne Normen-Anker. Rechtliche Pruefung bleibt anwaltliche Aufgabe.

# Optionaler Reiter Beteiligte

## Rolle und Fokus
Optionaler Reiter Beteiligte. Uebersicht aller Parteien mit Rolle, Funktion, Anteil und Kontaktdaten. Vorbereitung fuer Zustellung, Kommunikation und Sitzungsorganisation.

## Vorgehen

1. **Zwei Tabellenteile** — Gesellschafterseite (mit Anteil) und Vertragspartnerseite (Glaeubiger, Vermieter, Behoerden, Berater).
2. **Vertretungsregelung pro Partei** — HR-Eintrag oder Vollmacht; Vertretung allein oder gemeinschaftlich.
3. **Zustellbevollmaechtigte** — Insbesondere bei Auslandsparteien und Fonds-Strukturen.
4. **Kommunikationskanal-Praeferenz** — E-Mail vs. Brief vs. Datenraum-Notification (vertraglich vereinbart?).
5. **Konflikt- und Befangenheits-Hinweis** — Doppelmandate, Beraterueberschneidung, gemeinsame Vorstaende.

## Anwendungsbeispiel
LausitzStorage Beteiligten-Reiter: 4 Gesellschafter (Bauernfeind privat, NordCap Infrastructure Fund SCS Luxemburg, Stadtwerke Cottbus, LausitzWind GmbH), 6 Vertragspartner (LEAG, 50Hertz, LfU Brandenburg, ILB, NordCap als Glaeubiger separat, Notarin Dr. Krause). Zustellbevollmaechtigte NordCap-Side: Kanzlei Niederee in Frankfurt.

## Output-Module
- Beteiligten-Reiter mit Gesellschafter- und Vertragspartner-Teil
- Vertretungsspalte mit HR-Bezug oder Vollmachts-Datum
- Zustellbevollmaechtigten-Liste fuer den Skill `zugang-zustellung-pruefung`

## Grenzen
- **Keine rechtliche Wirksamkeitspruefung.** Subsumtion bleibt anwaltliche Aufgabe.
- **Hinweise, keine Befunde.** Markierungen muessen anwaltlich verifiziert werden.
- **Datenschutz und Berufsrecht.** Nutzung nur mit System, das DSGVO, § 203 StGB und §§ 43a, 43e BRAO erfuellt.
