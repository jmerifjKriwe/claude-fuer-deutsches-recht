...
| Skill | Beschreibung |
| --- | --- |
| `changes-beweislast-docx-erkennen` | Changes Beweislast Docx Erkennen: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `erzeugen-red-fassungen-sonderfall-felder` | Erzeugen RED Fassungen Sonderfall Felder: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `fuehren-interessen-mappen-nachfrage` | Fuehren Interessen Mappen Nachfrage: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `neue-rueckfragen-strippen` | Neue Rueckfragen Strippen: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
...
| `term-track-vertraege` | Term Track Vertraege: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
...
| `vaf-plausibilitaetscheck-termsheet` | VAF Plausibilitaetscheck Termsheet: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vaf-versionierung` | VAF Versionierung: bearbeitet den maßgeblichen Prüfpfad und erzeugt den nächsten belastbaren Output im Mandat. |
...
| `vertragsausfueller-mandantenkommunikation-redteam-qualitygate` | Mandantenkommunikation Redteam Qualitygate: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
...
| `vertragsausfueller-start-chronologie-fristen` | Start Chronologie Fristen: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
...
| `vertragsausfueller-vaf-batch-modus-docx-stripper-einfuehrung` | VAF Batch Modus Docx Stripper Einfuehrung: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vertragsausfueller-vaf-bsag-mietvertrag-klauselentscheidung` | VAF Bsag Mietvertrag Klauselentscheidung: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vertragsausfueller-vaf-feldinventar-fragebogen-input` | VAF Feldinventar Fragebogen Input: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vertragsausfueller-vaf-kommandocenter-mehrsprachige-vertraege` | VAF Kommandocenter Mehrsprachige Vertraege: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vertragsausfueller-vaf-quality-gate-redline-qa` | VAF Quality Gate Redline QA: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vertragsausfueller-vaf-template-erkennung-format-track-changes` | VAF Template Erkennung Format Track Changes: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
| `vorlagen-vertragsausfueller-vaf-altvertrag` | Vorlagen Vertragsausfueller VAF Altvertrag: bündelt 3 verwandte Prüffelder und erzeugt den nächsten belastbaren Output — nach Frist, Zuständigkeit, Beweislast und gewünschtem Ergebnis priorisiert. |
