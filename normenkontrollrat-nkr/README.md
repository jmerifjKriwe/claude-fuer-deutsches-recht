...
| Skill | Beschreibung |
| --- | --- |
| `alternativen-keine-aufgabe-kompetenz` | Alternativen Keine Aufgabe Kompetenz im Plugin Normenkontrollrat Nkr: prüft konkret Systematische Pruefung von Alternativen zur geplanten, Zeigt Aufgabe Zustaendigkeit Unabhaengigkeit und. Liefert priorisierten Output mit Norm-Pinpoints,... |
| `buerokratieabbau-katalog-buerokratiekosten-vs` | Buerokratieabbau Katalog Buerokratiekosten VS im Plugin Normenkontrollrat Nkr: prüft konkret Katalog konkreter Buerokratieabbau-Vorschlaege als, Trennt den engen Begriff Buerokratiekosten von dem. Liefert priorisierten Output mit Norm-Pi... |
| `einmalig-vs-erforderlichkeitspruefung-warum` | Einmalig VS Erforderlichkeitspruefung Warum im Plugin Normenkontrollrat Nkr: prüft konkret Trennscharfe Unterscheidung zwischen einmaligem, Erforderlichkeitspruefung als erster Pruefschritt jeder. Liefert priorisierten Output mit Norm-Pi... |
| `erfuellungsaufwand-buerger-grundbegriff` | Erfuellungsaufwand Buerger Grundbegriff im Plugin Normenkontrollrat Nkr: prüft konkret Trennt den Erfuellungsaufwand methodisch nach den drei, Definiert den Grundbegriff Erfuellungsaufwand nach NKRG GGO. Liefert priorisierten Output mit... |
| `eu-ebene-eu-richtlinien` | EU Ebene EU Richtlinien im Plugin Normenkontrollrat Nkr: prüft konkret Verortet den NKR im europaeischen Better-Regulation-Kontext, Detailskill EU-Richtlinienumsetzung und. Liefert priorisierten Output mit Norm-Pinpoints, Risikoampel und... |
| `evaluation-jahresbericht-fallzahlen` | Evaluation Jahresbericht Fallzahlen im Plugin Normenkontrollrat Nkr: prüft konkret Beschreibt Evaluierungspraxis ex-post-Pruefung und, Methodischer Fachmodul fuer Schaetzungen mit Bandbreiten. Liefert priorisierten Output mit Norm-Pinpoi... |
| `evaluierung-befristung-verfahrensgang` | Evaluierung Befristung Verfahrensgang im Plugin Normenkontrollrat Nkr: prüft konkret Praxis-Skill zur Empfehlung von Evaluierungsklauseln, Skizziert den Verfahrensgang eines Vorhabens von der. Liefert priorisierten Output mit Norm-Pinpoi... |
| `gleichstellungs-gendercheck-handelsregister` | Gleichstellungs Gendercheck Handelsregister im Plugin Normenkontrollrat Nkr: prüft konkret Fachmodul Gleichstellungs- und Gendercheck nach § 1 BGleiG, Fachmodul fuer Vorhaben mit Handelsregister-Bezug und. Liefert priorisierten Output mi... |
| `leitfaden-ermittlung` | Leitfaden Ermittlung im Plugin Normenkontrollrat Nkr: prüft konkret Strukturierter Ueberblick ueber den Leitfaden zur, KMU-Test (Small and Medium Enterprises Test) als. Liefert priorisierten Output mit Norm-Pinpoints, Risikoampel und näc... |
| `nachhaltigkeit-klimacheck-one` | Nachhaltigkeit Klimacheck ONE im Plugin Normenkontrollrat Nkr: prüft konkret Fachmodul Nachhaltigkeitspruefung und Klimacheck, One-in-one-out-Regel der Bundesregierung 2015 als. Liefert priorisierten Output mit Norm-Pinpoints, Risikoampe... |
...
| `nkr-digital-anschlussfaehigkeit-digitalcheck` | Digital Anschlussfaehigkeit Digitalcheck im Plugin Normenkontrollrat Nkr: prüft konkret Pruefskill Digitaltauglichkeit, Fachmodul OZG und Digitalcheck. Liefert priorisierten Output mit Norm-Pinpoints, Risikoampel und nächstem Schritt. |
...
| `orientierung-mandatsaufnahme-praktikabilitaet` | Orientierung Mandatsaufnahme Praktikabilitaet im Plugin Normenkontrollrat Nkr: prüft konkret Einstiegs-Skill fuer NKR-Pruefauftraege, Praktikabilitaetstest fuer den Vollzug. Liefert priorisierten Output mit Norm-Pinpoints, Risikoampel un... |
...
| `pruefumfang-was-standardkostenmodell-skm` | Pruefumfang WAS Standardkostenmodell SKM im Plugin Normenkontrollrat Nkr: prüft konkret Negativabgrenzung des NKR-Pruefumfangs, Beschreibt das Standardkostenmodell SKM als methodischen. Liefert priorisierten Output mit Norm-Pinpoints, Ri... |
| `stellungnahme-aufbau-ergebnis` | Stellungnahme Aufbau Ergebnis im Plugin Normenkontrollrat Nkr: prüft konkret Standardaufbau und Format einer NKR-Stellungnahme nach § 6, Schreibt den Schlussteil einer NKR-Stellungnahme. Liefert priorisierten Output mit Norm-Pinpoints, R... |
| `stellungnahme-evaluationsklausel` | Stellungnahme Evaluationsklausel im Plugin Normenkontrollrat Nkr: prüft konkret Fachmodul fuer den NKR-Empfehlungs-Baustein, Behandlung von Formulierungshilfen der Bundesregierung an. Liefert priorisierten Output mit Norm-Pinpoints, Risi... |
| `stellungnahme-grundsatzfeststellung` | Stellungnahme Grundsatzfeststellung im Plugin Normenkontrollrat Nkr: prüft konkret Schreibt den Eingangsabschnitt Grundsatzfeststellung einer, Beschaeftigt sich mit dem mahnenden Charakter der. Liefert priorisierten Output mit Norm-Pinpo... |
