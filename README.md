...
| **Fachanwalt für Steuerrecht** (nicht separat) | ⚠️ Es gibt **kein** eigenes Light-Touch-Plugin `fachanwalt-steuerrecht`. Steuerrecht ist konsolidiert im Power-Plugin [`steuerrecht-anwalt-und-berater`](./steuerrecht-anwalt-und-berater) — dort sind sowohl die Fachanwalts-Skills (Präfix `fa-`) als auch die Anwalts- (`anw-`) und Steuerberater-Workflows (`stb-`) enthalten. Hintergrund: Steuerrecht ist anders als die übrigen FAO-Felder zwischen Fachanwalt und Steuerberater eng verzahnt; ein gemeinsames Plugin vermeidet Doppelungen. |
